...
def convert_to_nimtype_string(vtype):
    """
    :param vtype:
    :return: その型に対応するNimにおける文字列表現
    """
    if vtype == float:
        return "float64"
    elif vtype == int:
...
def input_code(vtype, vname_for_input):
    if vtype == float:
        return '{name} = input().parseFloat'.format(name=vname_for_input)
    elif vtype == int:
        return '{name} = input().parseInt'.format(name=vname_for_input)
    elif vtype == str:
        return '{name} = input()'.format(name=vname_for_input)
    else:
        raise NotImplementedError
...
def generate_declaration(v):
    ...
    dim = len(v.indexes)
    typename = convert_to_nimtype_string(v.type)
    ...
    if dim == 0:
        ...
        type_template_after = " = newSeq[{type}]({size})".format(
            type=typename,
            size=v.indexes[0].zero_indexed().max_index)
    elif dim == 2:
        type_template_before = ""
        type_template_after = " = newSeqWith({row_size}, newSeq[{type}]({col_size}))".format(
            type=typename,
    ...
    line = "var {name}{constructor}{declaration}".format(
        name=v.name,
...
def generate_arguments(var_information):
    ...
    for name, v in var_information.items():
        ...
        if dim == 0:
            ...
            type_template = "seq[{type}]".format(type=typename)
        elif dim == 2:
            type_template = "seq[seq[{type}]]".format(type=typename)
        else:
            raise NotImplementedError

        formal_lst.append("{name}:{type}".format(
...
def generate_input_part(node, var_information, inputted, undeclared, depth, indexes):
    ...
    if node.pointers is not None:
        ...
        if node.index is None:
            ...
            # ループの開始
            lines.append("for {x} in {start}..{end}:".format(
                x=loopv,
...
def code_generator(predict_result=None):
    with open("{dir}/template.nim".format(dir=mydir), "r") as f:
        template = f.read()
    with open("{dir}/template_success.nim".format(dir=mydir), "r") as f:
        template_success = f.read()
    with open("{dir}/template_failure.nim".format(dir=mydir), "r") as f:
